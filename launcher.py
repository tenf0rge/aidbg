...
# opencode engine config: common (model/provider) + 3 permission tiers (agents)
_OPENCODE_CONFIG = ROOT / "configs" / "opencode" / "opencode.jsonc"
...
def run(*, engine: str, profile: str, wave: str | None, log: str | None,
        ...
        out: str | None, mode: str = "readonly") -> int:
    """Drive an LLM engine with the assembled prompt; emit the report it writes.

    `mode` selects the permission tier (readonly / safe-edit / edit). aidbg's
    debug path defaults to readonly; the others relax the read-only guarantee.
    """
    ...
    if engine == "opencode":
        # ship the engine config (common base + 3 permission tiers) into the run
        # dir so opencode reads it as project config; pick the tier with --agent.
        if _OPENCODE_CONFIG.is_file():
            shutil.copy(_OPENCODE_CONFIG, os.path.join(workdir, "opencode.jsonc"))
        output = ("最終成果物のMarkdownレポートを、次の正確なパスにだけ書き出すこと"
                  f"（他のファイルは書かない）: {report_path}")
        argv = [binary, "run", "--agent", mode, "--model", model, "{prompt}"]
        note = f"opencode ({model}, --agent {mode})"
    else:  # claude
        output = "完成したMarkdownレポートを最終メッセージとして出力すること（ファイルは書かない）。"
        # readonly: read-only Bash only. Relaxed tiers add the edit/write tools.
        allowed = ["Bash"] if mode == "readonly" else ["Bash", "Edit", "Write"]
        argv = [binary, "-p", "{prompt}", "--allowedTools", *allowed]
        note = f"claude (Claude Code, mode={mode})"
    ...
    if engine == "opencode":
        # opencode writes the report to the file; its stdout carries engine logs
        # (and, on failure, error dumps) — never treat that as the report.
        md = Path(report_path).read_text(encoding="utf-8") if os.path.exists(report_path) else ""
    else:  # claude returns the report as its final message on stdout
        md = proc.stdout.strip()
    if not md:
        print(f"[aidbg auto] no report produced ({engine}). engine output tail:", file=sys.stderr)
        print((proc.stderr or proc.stdout or "")[-800:], file=sys.stderr)
        return 5
